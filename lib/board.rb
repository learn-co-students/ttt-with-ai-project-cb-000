class Board
  ...
  def update(pos, playerObj)
      ...
      #input MUST be open cell, or current token
      #will be overwritten
  ...
  def valid_move?(pos)
      #returns true if position chosen is 0-8 and 
      #string contains no characters (only digits)
      #does NOT validate if cell chosen is empty
      #see taken?
      if pos.scan(/\D/).empty? && pos.to_i <=9 && pos.to_i >=1 && !taken?(pos.to_i)
         true
      else
          false
      end
